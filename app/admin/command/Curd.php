...
use \tools\CliEcho;
use app\admin\command\curd\BuildCurd;
...
class Curd extends Command
{
    protected function configure()
    {
        $this->setName('curd')
            ...
            ->addOption('menu', 'u', Option::VALUE_REQUIRED, '创建菜单', 0)
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        try {
            ...
            if (!$delete) {
                ...
                if($menu){
//                    创建菜单
                    $menuIndex=$build->controllerUrl.'/index';
                    $menuName=preg_replace('/表$/', '',$build->tableComment);

                    //                    判断有没有没有则创建
                    $count=Db::name('auth_rule')->where('href','=',$menuIndex)->count();
                    if($count<1){
                        Db::startTrans();
                        try {
                            //                        没有则创建
                            ...
                            if($id){
                                ...
                                Db::name('auth_rule')->insert([
                                    'href'=>$build->controllerUrl.'/delete',
                                    'title'=>'删除',
                                    'pid'=>$id,
                                    'type'=>1,
                                    'status'=>1,
