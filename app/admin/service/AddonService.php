...
class AddonService extends AbstractService
{
    ...
    //添加菜单
    public function addAddonMenu(array $menu,int $pid = 0,string $plugin=''){
        if(!$menu) return;
        if(!$plugin) return;
//        先清除原来的菜单
//        \think\facade\Db::name("auth_rule")->where("plugin",$plugin)->delete();
        foreach ($menu as $v){
    ...
    public function updateMenu(array $menu,int $pid = 0,string $plugin=''){
        if(!$menu) return;
        if(!$plugin) return;
        foreach ($menu as $v){
            ...
            try {
                ...
                $id=\think\facade\Db::name("auth_rule")->where( $where)->value('id');
                if ($id) {
                ...
                if ($hasChild && $id) {
                    $this->updateMenu($v['sublist'], $id,$plugin);
                }
            } catch (Exception $e) {
                throw new \Exception($e->getMessage());
            }
