...
class ContractReceivables extends Authority
{
    ...
    public function edit($id)
    {
        $prefix=getDataBaseConfig('prefix');
        $fields=Db::query('SELECT `name`,`xsname`,`rule`,`msg`,`field`,`edit_readonly`,`editinput`,`formtype`,`addinput` FROM `'.$prefix.'system_field` WHERE `edit`=1 AND `table`="crm_contract_receivables" order BY `sort` ASC,id ASC');
        $row = $this->model->field(array_unique(array_merge(array_column($fields, 'field'), ['id','owner_admin_id','check_status','numbering','customer_id','receivables_plan_id'])))->find($id);
