...
class Auth extends Authority
{
    ...
    public function getRoledata(){
        ...
        $roleList = \think\facade\Db::name('auth_role')->field('id,name')->where($where)->order('sort asc,id asc')->select()->toArray();
        $result = [];
    ...
    public function getGroupdata(){
        $groupList = AuthGroup::order('id asc')->select()->toArray();
        $tree = Leftnav::menu($groupList, '&nbsp;&nbsp;', 0, 0, 0);
        $result = [];
        foreach ($tree as $item) {
            $result[$item['id']] = $item['lefthtml'] . $item['title'];
        }

        // 非超级用户组限制可见范围
        if($this->admin['group_id'] > 1){
            $childGroupIds = (new AuthGroup())->getChildrenGroupIds($this->admin, false);
            if(!empty($childGroupIds) && is_array($childGroupIds)){
                $filtered = [];
                foreach($result as $id => $title){
                    if(in_array($id, $childGroupIds)){
                        $filtered[$id] = $title;
                    }
                }
                $result = $filtered;
...
//                哪些管理员id有查看权限
    ...
    public function adminEdit(){
        ...
        if($this->request->isPost()){
            ...
            $row['pwd']='';
            $this->success('获取成功','',['row'=>$row, 'groupids' => $groupids]);
        }
    }
    /*-----------------------用户组管理----------------------*/
    //用户组管理
