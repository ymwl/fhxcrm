...
class Field extends Common
{
    ...
    {
        parent::initialize();
        $this->dao=\think\facade\Db::name('module');
        $field_pattern = [
    ...
    public function edit(){
        if($this->request->isPost()){
            ...
            if(Db::name('module')->update($data)!==false){
                savecache('Module');
                return array('code'=>1,'url'=>myurl('index'),'msg'=>fy("Modification succeeded"));
            }else{
                return array('code'=>0,'url'=>myurl('index'),'msg'=>'修改失败!');
            }
        }else{
            $map['id'] = input('param.id');
            $info = \think\facade\Db::name('module')->field('id,title,name,description,listfields')->where($map)->find();
