...
class EmptyController extends Common{
    ...
    public function index(){
        if(Request::isAjax()){
            ...
            if (input('post.catid')) {
                $catids = Db::name('category')->where(array('pid'=>input('post.catid')))->column('id');
                if($catids){
            ...
            return View::fetch('content/index');
        }
    }
    ...
    public function edit(){
        ...
        View::assign( 'title', '编辑内容' );
        return View::fetch('content/edit');
    }
    function update(){
        $request = Request::instance();
        $controllerName = $request->controller();
        ...
        if (false !== $list) {
            if($controllerName=='Page'){
                ...
                $result['url'] = url($controllerName."/index",array('catid'=>$data['catid']));
            }
            //标签
            if(isset($data['tags'])){
    ...
    public function set_categorys($categorys = array()) {
        if (is_array($categorys) && !empty($categorys)) {
            foreach ($categorys as $id => $c) {
                ...
                $r = Db::name('category')->where("pid = $c[id]")->order('listorder ASC,id ASC')->select();
    ...
    public function listDel(){
        ...
        return ['code'=>1,'msg'=>fy('Delete succeeded').'！'];
    }
