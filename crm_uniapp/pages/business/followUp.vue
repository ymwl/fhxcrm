	...
	<view >
		<view class="set-box">
			<u-form :model="form" ref="uForm" :error-type="errorType">
				<u-form-item label="跟进商机:"  label-width="180" prop="name">
				...
				<!-- <u-form-item label="图片文件:"  label-width="180">
					<view class="u-upload">
						<view class="slot-btn" style="" v-if="videoFilePath != '' ">
							<video :src="videoFilePath" ></video>
							<view class="delete-icon" @tap.stop="deleteVideoFile">
								<u-icon class="u-icon" name="close" size="20" color="#fff"></u-icon>
							</view>
						</view>
						<view v-else class="slot-btn" @click="videoFile">
							<u-icon name="camera" size="60" color="#606266"></u-icon>
							<view class="text">上传图片/文件</view>
						</view>
					</view>
				</u-form-item> -->
			</u-form>
		...
		methods: {
			...
			submit() {
				...
				this.$refs.uForm.validate(valid => {
					if (valid) {
						// 图片和视频都选择了 先上传图片
						if(this.lists.length > 0 && this.videoFilePath) {
