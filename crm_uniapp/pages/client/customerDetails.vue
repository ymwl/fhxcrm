  ...
  <view class="container">
    ...
    <view class="notice">
      <view class="belong">
        ...
        <view :style="{color: customer.issuccess == 1 ? '#19be6b' :vuex_theme.color}">
          {{ customer.issuccess | changeStatus }}
          <u-icon name="arrow-right" :color="vuex_theme.color" :size="26"></u-icon>
        </view>
      </view>
      ...
      <view class="relation u-flex">
        <view class="left">手机：
          <text class="dial" @click="call(customer.phone)">{{ customer.phone }}</text>
        </view>
      ...
      <view class="bottom u-flex">
        ...
        <view class="right">负责人：{{ customer.pr_user }}</view>
      </view>
      <view class="more u-flex" @click="onLookMore">编辑客户
        <u-icon name="arrow-down" size="35"></u-icon>
      </view>
    </view>
    <!-- 工具项 -->
    <view class="region u-m-t-35">
      <view class="u-border-bottom">
        <u-tabs :list="tapList" bar-width="80" :active-color="vuex_theme.color" :is-scroll="false" :current="current"
        ...
        <view class="page-box">
          ...
          <block v-if="current == 0">
            <block v-if="recordList.length > 0">
              <view class="list-view" v-for="(item, index) in recordList" :key="index">
                ...
                <view class="item">
                  <view class="left">
                    <block v-if="item.attachs && item.attachs.length > 0">
                      <view class="attach-item" v-for="(file, x) in item.attachs" :key="x">
                        <!-- 图片附件 -->
                        <view v-if="file.isImage" class="image">
                          <u-image width="100%" height="100%" :src="file.url" @click="lookImges(file.url)"></u-image>
                        </view>
                        <!-- 办公文件附件 -->
                        <view v-else class="file-item" @click="openFile(file.url)">
                          <u-icon :name="getFileIcon(file.ext)" size="36" :color="getFileIconColor(file.ext)"></u-icon>
                          <text class="file-name u-line-1">{{ file.name }}</text>
                        </view>
                      </view>
                    </block>
                  </view>
                ...
                <view class="bottom">
                  ...
                  <view class="way">{{ item.record_type ? item.record_type : '--' }}</view>
                </view>
              </view>
          ...
          <block v-if="current == 1">
            <block v-if="contactList.length > 0">
              <view class="client" v-for="(item,index) in contactList" :key="index" @click="lookDetails(item)">
                <view class="client-content">
                  <fhx-field-display :fields="contactFields" :item="item"></fhx-field-display>
                </view>
            ...
            <block v-if="businessList.length > 0">
              <view class="list-view" v-for="(item,index) in businessList" :key="index" @click="lookDetails(item)">
                <view class="top">
                  ...
                  <view class="right u-flex" :style="{color: vuex_theme.color}">{{ item.is_end | is_endStatus }}
                    <u-icon name="arrow-right" :color="vuex_theme.color" :size="26"></u-icon>
                  </view>
                </view>
                <view class="item">
                ...
                <view class="bottom">
                  ...
                  <view class="btn entity" :style="{backgroundColor: vuex_theme.color}" @click.stop="follow(item.id)">
                    跟进
                  </view>
                </view>
              </view>
          ...
          <block v-if="current == 3">
            <block v-if="htList.length > 0">
              <view class="list-view" v-for="(item,index) in htList" :key="index" @click="lookDetails(item)">
                <view class="top">
                  <view class="left u-flex-1">{{ item.name }}</view>
                  <view class="right u-flex" :style="{color: vuex_theme.color}">{{ item.check_status | checkStatus }}
                    <u-icon name="arrow-right" :color="vuex_theme.color" :size="26"></u-icon>
                  </view>
                </view>
                <view class="item">
                  ...
                  <view class="content">
                    ...
                    <view class="u-flex">
                      ...
                      <view class="u-flex" v-if="renewShow(item)" @click.stop="onGetRenewPayurl(item.id)">
                        <u-icon class="u-p-r-1" name="renew" custom-prefix="custom-icon" :size="46"
                                :color="vuex_theme.color"></u-icon>
                        续费码
                      </view>
                    </view>
                  </view>
                ...
                <view class="bottom">
                  <view class="time">始止时间：{{ timeFormats(item.start_time) + '~' + timeFormats(item.end_time) }}
                  </view>
              ...
              <view class="list-view" v-for="(item,index) in hkList" :key="index" @click="lookDetails(item)">
                ...
                <view class="item">
                  ...
                  <view class="content u-flex">
                    <view class="type u-flex-1 u-m-r-20">备注: {{ item.remark ? item.remark : '--' }}</view>
                    <view class="u-flex"
                          v-if="item.pay_type == 2 && payConfig.renew_pay == '1' && item.pay_status == '0'"
                          @click.stop="onGetPayurl(item.id)">
                      <u-icon class="u-p-r-1" name="renew" custom-prefix="custom-icon" :size="46"
                              :color="vuex_theme.color"></u-icon>
                      收款码
                    </view>
...
<script>
import {attBaseUrl} from '@/common/config'
...
export default {
  ...
  methods: {
    ...
    // 确定转移客户
    onConfirm(val) {
      this.$u.post('crm.customer/alter_pr_user', {ids: this.customer_id, admin_id: val}).then(res => {
        if (res.code == 1) {
          // 提示
          uni.showToast({
            title: res.msg,
      ...
      switch (index) {
        ...
        case 1:
          uni.showModal({
            ...
            success: function (res) {
              if (res.confirm) {
                this.$u.post('crm.customer/to_move_gh', {
        ...
        case 3:
          uni.showModal({
            ...
            success: function (res) {
              if (res.confirm) {
                this.$u.post('crm.customer/delete', {
                  ids: this.customer_id
                }).then(res => {
                  if (res.code == 1) {
                    // 提示
    ...
    // 获取客户详情
    getData() {
      this.$u.get('crm.customer/edit', {id: this.customer_id, 'source': 'details'}).then(res => {
        if (res.code == 1) {
    ...
    getCustomerRecord(isNextPage, pages) {
      this.$u.get('crm.record/index', {
        ...
        if (res.code == 1) {
          // 解析每条记录的附件字段（管道符分隔字符串 -> 文件对象数组）
          if (res.data.rows && res.data.rows.length > 0) {
            res.data.rows.forEach(record => {
              record.attachs = this.parseAttachFiles(record.attachs)
            })
          }
          // 不够一页
    ...
    getContactFields() {
      this.$u.get('fields/get_fields', {table: 'crm_customer_contacts', source: 'index'}).then((res) => {
        if (res.code == 1) {
          this.contactFields = res.data.fields || []
        }
      })
    },
    // 获取商机
    getBusinessList(isNextPage, pages) {
      this.$u.get('crm.business/index', {
        sort_order: 'desc',
        customer_id: this.customer_id,
        offset: (pages || 0) * this.pageSize,
        limit: this.pageSize,
    ...
    getReceivables(isNextPage, pages) {
      this.$u.get('crm.contract_receivables/index', {
        sort_order: 'desc',
        customer_id: this.customer_id,
        offset: (pages || 0) * this.pageSize,
        limit: this.pageSize,
    ...
    // 解析附件字符串为文件对象数组
    parseAttachFiles(attachsStr) {
      if (!attachsStr || typeof attachsStr !== 'string' || attachsStr.trim() === '') {
        return []
      }
      const filePaths = attachsStr.split('|').filter(p => p.trim() !== '')
      const imageExts = ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'webp', 'svg']
      return filePaths.map(path => {
        const trimmedPath = path.trim()
        const fileName = this.getFileNameFromPath(trimmedPath)
        const ext = fileName.split('.').pop().toLowerCase()
        const isImage = imageExts.includes(ext)
        let fullUrl = trimmedPath
