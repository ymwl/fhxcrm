<template>
	<view>
		...
		<block v-for="(item,index) in fields" :key="index">
			...
			>
				<fa-check-radio :faList="item.content_list" v-model="forms[item.field]" :checkValue="item.value || item.defaultvalue"></fa-check-radio>
			</u-form-item>
			<!-- 单选框 -->
			<u-form-item
				:label-position="labelPosition"
			...
			>
				<fa-selectpages
					...
					:showValue="(forms[item.field] ? forms[item.field] : item.value) || item.defaultvalue"
					v-model="forms[item.field]"
				></fa-selectpages>
			</u-form-item>
			<!-- 关联多选 -->
