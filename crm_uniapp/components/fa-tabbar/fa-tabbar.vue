		...
		>
			...
			>
				<view :class="[tabbar.midButton && item.midButton ? 'u-tabbar__content__circle__button' : 'u-tabbar__content__item__button']">
					...
					></u-image>
					<!-- <u-icon
						:size="tabbar.midButton && item.midButton ? midButtonSize : tabbar.iconSize"
	...
	computed: {
		elIconPath() {
			return index => {
				...
				let path = this.$reuse.getPath(this.tabbar.list[index].path);
		...
		showTabbar() {
			if (this.tabbar.list) {
				...
				this.tabbar.list.forEach(item => {
					let path = this.$reuse.getPath(item.path);
					if (path == this.pageUrl || path == '/' + this.pageUrl) {
						status = true;
					}
				});
	...
	methods: {
		...
		switchTab(index) {
			...
			if (this.tabbar.list[index].path) {
				if (tabBarLinks.indexOf(this.tabbar.list[index].path) > -1) {
					// 跳转tabBar页面
					uni.switchTab({
					...
					// 跳转不是tabBar的页面
					this.$u.route(this.tabbar.list[index].path)
				}
			} else {
