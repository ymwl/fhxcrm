...
# 若未运行安装向导（如手动导入数据库），请手动设置一个独立的 64 位随机字符串，避免多站点共用相同密钥。
...
> **提示（JWT 密钥）**：安装向导在写入站点信息时，会自动生成一个独立的 64 位十六进制随机密钥（256 位）并写入 `.env` 的 `APP_KEY`，作为 API 移动端 JWT Token 的 HS256 签名密钥，确保每个部署的密钥唯一。已安装的老站点升级后不会重新运行安装向导，需手动在其 `.env` 中设置独立的 `APP_KEY`（密钥长度须 >= 32 字节）。
...
**访问地址：**
```
http://你的域名/index.php/cron/index?token=your_cron_token_here_2026
```
